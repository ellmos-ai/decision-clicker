...
def cmd_check(args: argparse.Namespace) -> int:
    ...
    print(f"  Desktop-Postfach: {len(offen_postfach)} noch nicht uebernommen"
...
def cmd_intake(args: argparse.Namespace) -> int:
    ...
    if not ergebnis:
        print("Desktop-Postfach: nichts zu uebernehmen.")
        return 0
    kopf = "WUERDE UEBERNEHMEN" if args.dry_run else "UEBERNOMMEN"
    ...
    print(f"{len(ergebnis)} Eintraege.")
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--check", action="store_true", help="nur Kette pruefen")
    _gemeinsam(parser)
    ...
    p_in = unter.add_parser("intake", help="Desktop-Postfach in die Kette uebernehmen")
